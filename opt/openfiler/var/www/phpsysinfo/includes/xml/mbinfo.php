...
// $Id: mbinfo.php,v 1.17 2006/08/26 10:19:46 bigmichi1 Exp $
...
function xml_mbinfo() {
	...
	if( sizeof( $arrBuff ) > 0 ) {
		...
		foreach( $arrBuff as $arrValue ) {
			...
			$_text .= "      <Div>" . htmlspecialchars( $arrValue['div'], ENT_QUOTES ) . "</Div>\n";
			$_text .= "     </Item>\n";
		}
		$_text .= "    </Fans>\n";
	}
...
function wml_mbtemp() {
	...
	for( $i = 1, $max = sizeof( $XPath->getDataParts( "/phpsysinfo/MBinfo/Temperature" ) ); $i < $max; $i++ ) {
		...
		if( $XPath->getData( "/phpsysinfo/MBinfo/Temperature/Item[" . $i . "]/Value" ) == 0 ) {
			...
			$_text .= "&nbsp;" . str_replace( "&ordm;", "", temperature( $XPath->getData( "/phpsysinfo/MBinfo/Temperature/Item[" . $i . "]/Value" ) ) ) . "</p>\n";
		}
	}
